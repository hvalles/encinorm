...
class Filter:
    ...
    @staticmethod
    def geo_within(lat_field, lon_field, lat, lon, radius_km) -> "Filter":
        """Filtro "cerca de": bounding-box de `radius_km` alrededor de (lat, lon).

        Aproximación rectangular portable (usa índices `(lat, lon)` si existen).
        Para distancia exacta, combina con `db.fn.geo_distance`. No resuelve el
        cruce del antimeridiano (longitud ±180).
        """
