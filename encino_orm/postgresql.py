...
class PostgresDb(Db):
    ...
    async def connect(self, **kwargs):
        ...
        # a `asyncpg.connect`; los kwargs del driver quedan limpios.
    ...
    async def _ensure_status_column(self):
        ...
        asyncpg autocommitea fuera de transacción, así que NO se llama
        `self.commit()` aquí (ejecutaría un `COMMIT` fuera de transacción). El
        guard es "verify-then-swallow": si el `ALTER` falla por una carrera
