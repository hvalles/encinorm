...
def is_mariadb(db) -> bool:
    return engine_of(db) is Engine.MARIADB


def is_mssql(db) -> bool:
    return engine_of(db) is Engine.MSSQL


def is_oracle(db) -> bool:
    return engine_of(db) is Engine.ORACLE
