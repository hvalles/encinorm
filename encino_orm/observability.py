...
class OtelQueryTracer:
    ...
    Requiere `opentelemetry-api` (no es dependencia de encino_orm). El
    `TracerProvider` (SDK/exportador) lo configura la aplicación; encino_orm solo
    crea los spans.

    Uso:

    ```python
    tracer = OtelQueryTracer(tracer_name="encino_orm")
    tracer.record("sqlite", "fetch_all", sql, params, 0.012, rows=10)
    ```
    """

    def __init__(self, tracer_name: str = "encino_orm", collect_metrics=True):
