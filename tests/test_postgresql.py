...
async def pg_connected_db():
    ...
    try:
        ...
        engine_unavailable("postgresql", e)
...
_PARITY_DDL = (
    ...
    "id SERIAL PRIMARY KEY, nombre VARCHAR(50), monto DOUBLE PRECISION, "
...
class TestPostgresBuildersAndQueries:
    @pytest.mark.asyncio
    async def test_insert_execute_and_execute_insert(self, pg_connected_db):
        db = pg_connected_db
        await _reset(
...
class TestPostgresParity:
    ...
    @pytest.mark.asyncio
    async def test_count_paginate_and_list_tables(self, pg_connected_db):
        db = pg_connected_db
    ...
    @pytest.mark.asyncio
    async def test_list_tables_filtrado_por_nombre(self, pg_connected_db):
    ...
    @pytest.mark.asyncio
    async def test_query_builder_aggregates(self, pg_connected_db):
        db = pg_connected_db
        await _reset(db, "test_parity", _PARITY_DDL)
        await _seed_parity(db)
    ...
    @pytest.mark.asyncio
    async def test_query_builder_limit_first_exists(self, pg_connected_db):
    ...
    @pytest.mark.asyncio
    async def test_execute_insert_characterization(self, pg_connected_db):
        db = pg_connected_db
        await _reset(db, "test_parity", _PARITY_DDL)
