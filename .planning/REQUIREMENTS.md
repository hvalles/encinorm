...
- [ ] **CI-07**: El escaneo de dependencias/vulnerabilidades (`uv lock --check`, `uv audit` o `pip-audit`) corre en CI
...
- [ ] **DIAL-05**: `Query` es correcto: sin sentinel frágil `{0}`, inmutable/hashable, y `rebind` funciona
...
- [ ] **POOL-02**: `acquire()` es libre de carreras (reserva-antes-de-await) y nunca supera `max_size`
- [ ] **POOL-03**: `last_id` se captura **dentro** del insert (`RETURNING`/`SCOPE_IDENTITY`/`lastrowid` inmediato) por conexión/tarea; el `last_id()` post-hoc queda deprecado
- [ ] **POOL-04**: Existe una política `reset_on_release` configurable con rollback por defecto, más warning de deprecación y entrada de CHANGELOG
