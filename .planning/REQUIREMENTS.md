...
- [x] **RESL-02**: `_with_reconnect` reconecta una sola vez y **solo** cuando no hay transacción abierta
- [x] **RESL-03**: Las conexiones directas soportan `pre_ping` y `max_connection_lifetime`
- [x] **RESL-04**: Existe una taxonomía pública de errores que traduce excepciones de driver a excepciones de la librería
...
- [x] **CFG-03**: Los handlers generados con `exec()` se sustituyen por closures/`__signature__` en un refactor que preserva comportamiento, con snapshot OpenAPI antes/después
- [x] **CFG-04**: `build_schema` de GraphQL usa un namespace por build y deja de mutar el namespace del módulo
...
- [x] **REL-02**: Se publica 0.3.0rc1 antes de 0.3.0
- [x] **REL-03**: La publicación usa OIDC trusted publishing; `PYPI_API_TOKEN` se elimina y el entorno `pypi` está protegido
