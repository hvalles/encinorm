...
- **D-11:** **Solo la clave afectada**, sin invalidación de namespace. `CachedModel.load()` solo cachea por clave de PK (`_cache_key(keys)`), así que no existen entradas por filtro que haya que barrer.
  - **D-11 SUPERSEDED (ronda 2 de gap-closure, plan `03-06`):** la PREMISA de D-11 ("`load()` solo cachea por clave de PK") es **FALSA**: `load(keys=...)` es público y cacheaba bajo claves de LECTURA arbitrarias, así que un write por una clave distinta dejaba la entrada de la PK obsoleta (CR-01 residual, reproducido A2/B2/C2 en `03-VERIFICATION-FINAL.md`). La decisión se mantiene (solo la clave afectada, sin namespace) pero su premisa se convierte en **invariante**: el dominio de caché pasa a ser canónico (SOLO la PK de la fila) y toda escritura resuelve la PK real de la fila afectada antes de invalidar (de la instancia si las claves de escritura son la PK; de la BD con un SELECT ligado y con scope si no). Consecuencia documentada: una lectura no-PK (`load(keys=['rfc'])`) deja de acierto en caché (consulta la BD y recachea bajo la PK). Esto elimina por construcción el residual inverso (`load(keys=['rfc'])` seguido de `update()` por PK) en vez de documentarlo como irresoluble. D-12, D-15 y D-16 no cambian.
