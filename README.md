...
async def main():
    ...
    await User(db).create_table()            # genera el DDL y lo aplica

    u = User(db, name="Ana", age=30)
    await u.insert()                          # INSERT (id auto-incremental)

    found = await User(db, id=u.id).load()    # SELECT por clave primaria
    assert found.name == "Ana"

    adults = await User(db).search(Filter.ge("age", 18))  # SELECT ... WHERE age >= 18
    assert len(adults) == 1
